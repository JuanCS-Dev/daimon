...
// @ts-expect-error - maath doesn't have proper types
...
// Pre-generated deterministic wave positions
const NEURAL_WAVE_POSITIONS = (() => {
  const data = new Float32Array(300 * 3);
  for (let i = 0; i < 300; i++) {
    const angle = (i / 300) * Math.PI * 2;
    // Use seeded random based on index
    const seed = Math.sin(i * 12.9898) * 43758.5453;
    const r = 1.2 + (seed - Math.floor(seed)) * 0.3;
    data[i * 3] = Math.cos(angle) * r;
    const ySeed = Math.sin((i + 100) * 12.9898) * 43758.5453;
    data[i * 3 + 1] = ((ySeed - Math.floor(ySeed)) - 0.5) * 0.1;
...
function NeuralWave() {
  ...
  const positions = useMemo(() => NEURAL_WAVE_POSITIONS, []);
